...
import css from './CalcPage24.module.css'
...
const CalcPage24 = () => {
    ...
    const {tz,priceMax, setPriceMax,priceMin,
        ...
        setLogin,setFilter,setHistory,setFavourites,setSale,setWarehouse,setRaiting,setTz} = useContext(MyContext); 

    let message = `<b>Заявка с сайта!</b>\n`
    message += `<b>Номер: ${number}</b>\n`
    message += `<b>Интернет магазин</b>\n`
    ...
    if(search){message += `✔  Поиск по магазину ${search}руб.\n`}
    if(login){message += `✔  Регистрация, авторизация и личный кабинет. ${login}руб.\n`}
    if(filter){message += `✔  Фильтрация товаров ${filter}руб.\n`}
    ...
    if(warehouse){message += `✔  Информация о наличии на складе ${warehouse}руб.\n`}
    if(raiting){message += `✔  Рейтинг товара (звезды) ${raiting}руб.\n`}
        ...
        <motion.div ref={windRef} initial={{opacity:0}} whileInView={{opacity:1}} transition={{duration:.3}} className={css.wind1}>
            ...
            <div className={css.nestedContainer1} ref={modal1Ref} >
                        ...
                        {api?<div className={css.tz}>
                            ...
                            {basket?<div className={css.tz}>
                                <span className={css.tzText}>✔  Корзина<li>{basket}руб.</li></span>      
                            </div>:<></>}
                            {search?<div className={css.tz}>
                                <span className={css.tzText}>✔  Поиск по магазину<li>{search}руб.</li></span>      
                            </div>:<></>}
                            ...
                            {filter?<div className={css.tz}>
                                <span className={css.tzText}>✔  Фильтрация товаров<li>{filter}руб.</li></span>      
                            </div>:<></>}
                            {history?<div className={css.tz}>
                                <span className={css.tzText}>✔  История покупок<li>{history}руб.</li></span>      
                            </div>:<></>}
                            {favourites?<div className={css.tz}>
                                <span className={css.tzText}>✔  Избранные товары<li>{favourites}руб.</li></span>      
                            </div>:<></>}
                            {sale?<div className={css.tz}>
                                <span className={css.tzText}>✔  Гибкая система скидок<li>{sale}руб.</li></span>      
                            </div>:<></>}
                            {warehouse?<div className={css.tz}>
                                <span className={css.tzText}>✔  Информация о наличии на складе<li>{warehouse}руб.</li></span>      
                            </div>:<></>}
                            {raiting?<div className={css.tz}>
                                <span className={css.tzText}>✔  Рейтинг товара (звезды)<li>{raiting}руб.</li></span>      
                            </div>:<></>}
                    </div>
