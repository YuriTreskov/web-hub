import css from './CardPageMobile5.module.css'
...
const CardPageMobile5 = () => {
    const width = window.screen.width
    if (width>768){
        return ( 
            <>
            <section className={css.container}>
                <motion.h2 initial={{opacity:0}} whileInView={{opacity:.8}} transition={{duration:1}} className={css.header}>8 ШАГОВ ПРИ РАЗРАБОТКЕ ИНДИВИДУАЛЬНОГО ПРОЕКТА</motion.h2>
        ...
        return ( 
            ...
            index % 2 ? (
            <motion.div initial={{ x: -200 }}   whileInView={{ x: 0 }} className={css.card} >
                <span className={css.cardNumber}>{item.number}</span>
                <div className={css.cardContent}>
                    <h3 className={css.cardHeader}>{item.header}</h3>
                    <p className={css.cardFullText}>{item.content}</p>
...
export default CardPageMobile5;