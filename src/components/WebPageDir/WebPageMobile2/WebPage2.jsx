...
<section className={css.container}>
    ...
    <motion.h2 initial={{opacity:0}} whileInView={{opacity:1}} transition={{duration:1}} className={css.header}>Какие решения мы предлагаем?</motion.h2>
    <div className={css.cardContainer}>
        {lst.map((item,index)=>(
            <motion.div initial={{opacity:0}} whileInView={{opacity:1}}  className={css.card}>
            <div className={css.imgContainer}>
                ...
                <p className={css.fullText}>{item.description}</p>
                    <motion.div initial={{y:100}} whileInView={{y:0}} className={css.buttonContainer}>
                        <Link className={css.cardButtonLink}  to={item.link}>Подробнее</Link>
                        <Link className={css.cardButtonLink}  to='/calc'>Калькулятор</Link>
                    </motion.div>
                </div>
            </motion.div> 
        ))}
